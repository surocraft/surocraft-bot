...
module.exports.run = async (bot, message, args) => {
  ...
  const msg = await message.reply({ content: `Jsi si jistý, že chceš odeslat notifikaci k hlasování do <#921803832667832380> a zmínit roli?`, components: [row] });
  ...
  collector.on('collect', async i => {
    if (i.customId === 'pushVote') {
      ...
      await msg.edit({ content: `Notifikace odeslána!`, components: [] });
    }
  });
